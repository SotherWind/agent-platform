...
class LoggingTracer extends Tracer {
    ...
    const interesting = ["triage", "specialist", "tools", "generate", "review", "escalate"];
    if (interesting.includes(done.stage)) {
      ...
      if (attrs.reviewPassed !== undefined) parts.push(`reviewPassed=${attrs.reviewPassed}`);
      if (attrs.reviewViolations) parts.push(`violations=${String(attrs.reviewViolations)}`);
 ...
 */
async function resolveVectorStore(production: boolean, publications: KnowledgePublicationStore): Promise<VectorStoreType | undefined> {
  if (!process.env.QDRANT_URL && !process.env.QDRANT_API_KEY) {
    if (production) throw new Error("Production requires Qdrant configuration.");
    console.log("[boot] 未配置 Qdrant：检索为空（安全降级），图仍可直答");
    return undefined;
  }
  try {
    const store = await import("@agent-platform/rag-boot").then((m) => m.createVectorStore({ publications }));
    console.log(`[boot] Qdrant 已连接: ${process.env.QDRANT_URL ?? "(由 API key 推断)"}`);
    return store;
  } catch (error) {
    if (production) throw error;
    const message = error instanceof Error ? error.message : String(error);
    console.warn(`[boot] ⚠️ Qdrant 连接失败，检索降级为空（不影响对话启动）：${message}`);
    console.warn("[boot] ⚠️ 请确认 qdrant.exe 已启动（默认 6333 端口）后重启本服务以恢复知识检索。");
...
async function main(): Promise<void> {
  ...
  const persistence = createPersistence(config.dataDir, { localCrm: !config.businessModule });
  const authService = new AuthService(config);
  const gateway = buildAccessGateway(authService, {
    environment: config.environment, idempotency: persistence.entry, sessionBindings: persistence.sessions,
  });
  ...
  const audit = (entry: unknown) => console.log("[audit]", JSON.stringify(redactObject(entry)));
  const ticketService = new TicketService({ store: persistence.tickets });
  const proposalService = new ProposalService({
    secret: config.proposalSecret, store: persistence.proposals, onAudit: audit,
  });
  const signalBus = new ActionSignalBus({ store: persistence.signals, onAudit: audit });
  ...
  const dispatcher = new ActionDispatcher({
    proposals: proposalService, signals: signalBus, tools,
    idempotency: persistence.tools, guardrails: actionGuardrails, audit,
  });

  console.log("[boot] 正在构建智能客服图…");
  const vectorStore = await resolveVectorStore(config.environment === "production", persistence.publications);
  ...
  if (config.specialistPolicy === "skipSingleCategory") {
    console.log("[boot] 专家直通已开启：单类别查询跳过 specialist/orchestration（省一次 LLM 调用）");
  }
  ...
  const flush = () => {
    if (delivery) return;
    delivery = dispatcher.flush().then((results) => {
      for (const result of results) if (!result.ok) console.warn("[action]", redactObject(result));
  ...
  server.listen(config.port, () => {
    console.log(`[boot] 智能客服 server 已启动: http://localhost:${config.port}`);
    console.log(`[boot] 账号 ${config.users.length} 个，系统 token ${config.systemTokens.length} 个`);
    console.log(
